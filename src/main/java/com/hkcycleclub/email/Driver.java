...
//            Session session = createSessionTest();
            ...
            sendEmail(session, "H&K Nutters Ride 2024");
    ...
    private static void sendEmail(Session session, String subject) throws Exception {
        List<Recipient> recipients = RecipientParser.parseCsv("/list-nutters-ride.csv");
        ...
        for (Recipient recipient : recipients) {
            Message msg = generateEmailMessage(recipient.getEmailAddress(), recipient.getFullName(), session, subject);
            try {
                Transport.send(msg);
        ...
        failed.forEach(recipient -> System.out.println("failed to sent to: " + recipient.getFullName() + ", " + recipient.getEmailAddress()));

    }


    private static Message generateEmailMessage(String toEmail, String toName, Session session, String subject) throws Exception {
